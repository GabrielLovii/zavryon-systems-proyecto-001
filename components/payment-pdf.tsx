...
export function openPaymentPrintWindow(state: DemoState, paymentId: string) {
  ...
  const rows = [['Proveedor', supplier?.name], ['Pedido', order?.id], ['Recepción', reception?.id], ['Fecha', dateEs(payment.date)], ['Importe', money(payment.amount)], ['Caja', payment.cashBox], ['Método', payment.method], ['Estado', payment.status], ['Referencia', payment.reference], ['Responsable', payment.responsible], ['Notas', payment.notes]].map(([label, value]) => `<tr><th>${escapeHtml(String(label))}</th><td>${escapeHtml(display(value))}</td></tr>`).join('');
...
export function PaymentPdf({ state, paymentId, onClose }: { state: DemoState; paymentId: string | null; onClose: () => void }) {
  ...
  const share = async () => {
    const text = [`Pago ${payment.id}`, `Proveedor: ${display(supplier?.name)}`, `Pedido: ${display(order?.id)}`, `Fecha: ${dateEs(payment.date)}`, `Importe: ${money(payment.amount)}`, `Caja: ${display(payment.cashBox)}`, `Método: ${display(payment.method)}`, `Referencia: ${display(payment.reference)}`, `Notas: ${display(payment.notes)}`].join('\n');
  ...
  return <div className="print-overlay" role="dialog" aria-modal="true" aria-label={`Exportar ${payment.id}`}>
    ...
    <article className="print-document payment-print-document">
      <header className="print-header"><div className="print-brand"><img src="/icon.svg" alt="Logo" className="print-logo" /><div><div className="print-company">{state.config.companyName}</div><h1>Comprobante de pago</h1><p>{state.config.clientName} · documento individual</p></div></div><div className="print-order-id"><span>ID del pago</span><strong>{payment.id}</strong></div></header>
