...
export function Sidebar({ active, onNavigate, state, update }: { active: Section; onNavigate: (section: Section) => void; state: DemoState; update: (change: (state: DemoState) => DemoState) => void }) {
  ...
  return <>
    <button aria-label="Abrir menú" onClick={() => setOpen(true)} className="fixed left-4 top-4 z-20 rounded-lg bg-ink p-2 text-white lg:hidden"><Bars3Icon className="h-6 w-6" /></button>
    {open && <button aria-label="Cerrar menú" className="fixed inset-0 z-30 bg-ink/30 lg:hidden" onClick={() => setOpen(false)} />}
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-64 flex-col bg-ink text-white transition-transform lg:static lg:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
